...
public class GeneInfoModule extends CalcModule implements ModernClickListener {
	...
	public static final Path RES_DIR = 
			PathUtils.getPath("res/modules/geneinfo/");

	private static final Path HUMAN_GENE_FILE = 
			RES_DIR.resolve("ucsc_refseq_hg19.txt.gz");

	private static final Path MOUSE_GENE_FILE = 
			RES_DIR.resolve("ucsc_refseq_mm10.txt.gz");
	...
	private void addInfo() throws IOException, ParseException {
		...
		Map<String, Set<Strand>> strandMap = 
				DefaultHashMap.create(new TreeSetCreator<Strand>());
		...
		// Load Both files
		loadFile(HUMAN_GENE_FILE, 
				chrMap,
				strandMap,
				startMap, 
				endMap);
		
		loadFile(MOUSE_GENE_FILE, 
				chrMap,
				strandMap,
				startMap, 
				endMap);
		...
		DataFrame ml = 
				DataFrame.createDataFrame(m.getRows(), m.getCols() + 4);
