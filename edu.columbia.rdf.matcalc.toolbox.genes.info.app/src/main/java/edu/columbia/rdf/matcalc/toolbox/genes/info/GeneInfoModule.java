...
public class GeneInfoModule extends CalcModule implements ModernClickListener {
  ...
  private static final Path HUMAN_GENE_FILE = RES_DIR.resolve("ucsc_refseq_hg19.txt.gz");

  private static final Path MOUSE_GENE_FILE = RES_DIR.resolve("ucsc_refseq_mm10.txt.gz");
  ...
  public void init(MainMatCalcWindow window) {
    ...
    RibbonLargeButton button = new RibbonLargeButton("Gene Info", UIService.getInstance().loadIcon("gene_info", 24));

    button.addClickListener(this);
    mWindow.getRibbon().getToolbar("Genomic").getSection("Annotation").add(button);
  }
  ...
  private void addInfo() throws IOException, ParseException {
    ...
    Map<String, Set<String>> chrMap = DefaultHashMap.create(new TreeSetCreator<String>());

    Map<String, Set<Strand>> strandMap = DefaultHashMap.create(new TreeSetCreator<Strand>());

    Map<String, List<Integer>> startMap = DefaultHashMap.create(new ArrayListCreator<Integer>());

    Map<String, List<Integer>> endMap = DefaultHashMap.create(new ArrayListCreator<Integer>());
    ...
    for (int i = 0; i < m.getRows(); ++i) {
      ...
      if (chrMap.containsKey(id)) {
        ...
        ml.set(i, m.getCols() + 1, Join.onSemiColon().values(Strand.toChar(strandMap.get(id))));
  ...
  private static void loadFile(Path file, Map<String, Set<String>> chrMap, Map<String, Set<Strand>> strandMap,
      Map<String, List<Integer>> startMap, Map<String, List<Integer>> endMap) throws IOException, ParseException {
    String line;
    List<String> tokens;
    ...
    try {
      ...
      while ((line = reader.readLine()) != null) {
        ...
        // refseq
        addEntry(tokens.get(0), chr, strand, start, end, chrMap, strandMap, startMap, endMap);

        // entrez
        addEntry(tokens.get(1), chr, strand, start, end, chrMap, strandMap, startMap, endMap);

        // symbol
        addEntry(tokens.get(2), chr, strand, start, end, chrMap, strandMap, startMap, endMap);
      }
    } finally {
      reader.close();
    }
  ...
  private static void addEntry(String name, String chr, Strand strand, int start, int end,
      Map<String, Set<String>> chrMap, Map<String, Set<Strand>> strandMap, Map<String, List<Integer>> startMap,
